    ...
    return Scaffold(
      body: SafeArea(child: Column(children: [
        ...
        Container(
          ...
          child: Row(children: [
            Container(width: 44, height: 44, decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: AppColors.tealLt, border: Border.all(color: AppColors.teal.withOpacity(0.25))),
              child: Icon(Icons.auto_awesome, color: AppColors.teal, size: 22)),
            SizedBox(width: 12),
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('AI Ассистент', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.text1)),
        ...
        Expanded(child: _msgs.isEmpty
          ? Center(child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
              Container(width: 80, height: 80, decoration: BoxDecoration(borderRadius: BorderRadius.circular(24), color: AppColors.tealLt),
              ...
              Wrap(alignment: WrapAlignment.center, spacing: 8, runSpacing: 8, children: _tips.map((t) => GestureDetector(
                ...
                child: Container(padding: EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: BoxDecoration(color: AppColors.teal.withOpacity(0.07), borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.teal.withOpacity(0.15))),
                  child: Text(t, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.text1))),
              )).toList()),
            ])))
          : ListView.builder(controller: _scroll, padding: EdgeInsets.all(16), itemCount: _msgs.length + (_loading ? 1 : 0), itemBuilder: (ctx, i) {
              if (i == _msgs.length) return Align(alignment: Alignment.centerLeft, child: Container(margin: EdgeInsets.only(top: 8), padding: EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.surface2, borderRadius: BorderRadius.circular(16)),
              ...
              return Padding(padding: EdgeInsets.only(bottom: 12), child: Column(crossAxisAlignment: isUser ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [
                if (!isUser) Padding(padding: EdgeInsets.only(bottom: 4), child: Row(children: [
                  Container(width: 24, height: 24, decoration: BoxDecoration(borderRadius: BorderRadius.circular(7), color: AppColors.tealLt), child: Icon(Icons.auto_awesome, size: 12, color: AppColors.teal)),
                ...
                Container(constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width * 0.78), padding: EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                  decoration: BoxDecoration(color: isUser ? AppColors.teal : AppColors.surface2, borderRadius: BorderRadius.circular(16).copyWith(bottomRight: isUser ? Radius.circular(4) : null, bottomLeft: !isUser ? Radius.circular(4) : null),
        ...
        Container(padding: EdgeInsets.fromLTRB(12, 8, 12, 12), decoration: BoxDecoration(color: Theme.of(context).colorScheme.surface, border: Border(top: BorderSide(color: AppColors.border))),
          child: Row(children: [
            ...
            GestureDetector(onTap: _send, child: AnimatedContainer(duration: Duration(milliseconds: 200), width: 44, height: 44,
              decoration: BoxDecoration(color: _ctrl.text.trim().isNotEmpty && !_loading ? AppColors.teal : AppColors.surface2, borderRadius: BorderRadius.circular(12)),
